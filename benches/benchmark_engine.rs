...
use kvs::engines::{KvsEngine, KvStore, SledKvsEngine};
...
pub fn criterion_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("kvs set 10", |b| b.iter(|| kvs_set(key_value_pairs.clone(),&mut kv_store)));
    ...
    sled_group.bench_function("sled set 100",|b|b.iter(|| sled_set(key_value_pairs.clone(), &mut sled_engine)));
    sled_group.bench_function("sled get 100",|b|b.iter(|| sled_get(key_value_pairs.clone(), &mut sled_engine)));
    sled_group.finish();
